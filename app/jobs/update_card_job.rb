class UpdateCardJob < ApplicationJob
  ...
  def perform(change, card_id, new_status)
    ...
    if change == "action_move_card_from_list_to_list"
      ...
      ProgressModule::UpdateUserStoryStatusService.new(targeted_task.user_story).call
      ProgressModule::UpdateSprintStatusService.new(targeted_task.user_story.sprint).call
    end
  end
end
