class UserStoryProgressService
  ...
  def calcul
    ...
    repartition[:waiting_for_client] = @tasks.where(current_status: "waiting client").count
    repartition[:to_review_debug]    = @tasks.where(current_status: "to_review_debug").count
